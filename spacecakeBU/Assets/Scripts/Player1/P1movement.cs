...
public class P1movement : MonoBehaviour
{
    ...
    private void playerCutscene() {
        Vector2 posRocket = playerrocket.GetComponent<P1rocket>().defaultPosition;

        if(position.x - .04 <= posRocket.x) {
            position.x = posRocket.x;
            transform.localScale = new Vector3(-1, 1, 1);
            spriteRenderer.enabled = false;
            playerrocket.GetComponent<P1rocket>().animationPlaying = false;
            playerrocket.GetComponent<P1rocket>().playAnimation();
        }
        else {
            position.x -= .04f;
            transform.localScale = new Vector3(-1, 1, 1);
    ...
    {
        if (Input.GetKey(KeyCode.A) && !Input.GetKey(KeyCode.D) && allowMovement)
        {
            position.x -= calculateMovementSin();
            transform.localScale = new Vector3(-1, 1, 1);
            animspeed = 1;
        }
        if (Input.GetKey(KeyCode.D)&& !Input.GetKey(KeyCode.A) && allowMovement)
        {
            position.x += calculateMovementSin();
            transform.localScale = new Vector3(1, 1, 1);
            animspeed = 1;
    ...
    {
        if (Input.GetKeyDown(KeyCode.W) && isOnGround && allowMovement)
        {
            isOnGround = false;
            jumping = true;
            jumpFactor = 60;
